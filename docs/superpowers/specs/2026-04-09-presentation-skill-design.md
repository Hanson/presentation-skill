...
10. 生成 Makefile deploy target（首次部署时，方便后续 make deploy）
```
...
首次部署成功后，skill 自动在用户项目根目录生成或追加 Makefile `deploy` target，方便后续直接 `make deploy` 重新部署，无需再通过 Claude。

### 行为规则

```
1. 部署成功后，检查项目根目录是否有 Makefile
2. 如果 Makefile 不存在 → 创建 Makefile
3. 如果 Makefile 存在但没有 deploy target → 追加 deploy target
4. 如果 Makefile 已有 deploy target → 不覆盖，跳过（尊重用户自定义）
5. deploy target 中的命令使用本次部署实际检测到的参数（包管理器、服务器信息等）
```
